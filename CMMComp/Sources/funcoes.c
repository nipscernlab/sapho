...
void vcall(int id)
{
    ...
    if (get_npar(p_test) != get_npar(v_fpar[id])) // p_test tem a lista de par na chamada e v_fpar na declaracao
        fprintf(stderr, "Erro na linha %d: olha lá direito quantos parâmetro tem a função %s.\n", line_num+1, rem_fname(v_name[id], fname));
...
int fcall(int id)
{
    ...
    {
        fprintf(stderr, "Erro na linha %d: lista de parâmetro da função %s difere da original.\n", line_num+1, v_name[id]);
        return 0;
    }
