...
void declar_ret(int et, int ret)
{
    ...
    if (v_type[fun_id1] == 6)
        fprintf (stderr, "Erro na linha %d: valor de retorno em fun��o void? viajou!\n", line_num+1);

    // testa se esta dentro de um if/else
    if ((get_if() > 0) && (v_type[fun_id1] != 6))
        fprintf(stdout, "Cuidado na linha %d: usar return dentro de if/else pode dar pau, caso voc� esque�a em algum lugar!\n", line_num+1);
    ...
    if ((left_type == 7) && (righ_type == 2))
    {
        if (prtype == 0)
        {
            fprintf(stdout, "Aten��o na linha %d: vai mesmo retornar float para int na fun��o %s? Vou meter um monte de instru��es assembly pra isso?\n", line_num+1, v_name[fun_id1]);

            if (is_macro() == 0) fprintf(f_asm, "CALL float2int\n");
            f2i = 1;
        }
        else
            fprintf(stdout, "Aten��o na linha %d: convertendo float para int no retorno da fun��o %s.\n", line_num+1, v_name[fun_id1]);
    }
    ...
    {
        fprintf(stdout, "Aten��o na linha %d: convertendo int para float no retorno da fun��o %s.\n", line_num+1, v_name[fun_id1]);
...
void declar_ret_cmp(int et)
{
    ...
    {
        fprintf (stdout, "Aten��o na linha %d: pra converter de int pra comp, eu vou jogar 0 na parte imagin�ria.\n", line_num+1);
    ...
    {
        fprintf (stdout, "Aten��o na linha %d: pra converter de float pra comp, eu vou jogar 0 na parte imagin�ria.\n", line_num+1);
...
void func_ret(int id) // id -> id da funcao atual
{
    ...
    if ((v_type[id] != 6) && (ret_ok == 0))
        fprintf (stderr, "Erro na fun��o %s: cad� o retorno pra essa fun��o?\n", v_name[id]);
...
void void_ret()
{
    ...
    if (v_type[fun_id1] != 6)
        fprintf (stderr, "Erro na linha %d: cad� o valor de retorno da fun��o?\n", line_num+1);
...
void vcall(int id)
{
    ...
    if  (v_type[id] < 6)
    {
        fprintf(stderr, "Erro na linha %d: cad� essa fun��o %s?\n", line_num+1, rem_fname(v_name[id], fname));
        return;
    }
    ...
    {
        fprintf(stderr, "Erro na linha %d: olha l� direito quantos par�metros tem a fun��o %s.\n", line_num+1, rem_fname(v_name[id], fname));
    }
...
int fcall(int id)
{
    if (v_type[id] == 6)
    {
        fprintf (stderr, "Erro na linha %d: olha l� a funcao %s, voc� vai ver que ela nao retorna nada.\n", line_num+1, v_name[id]);
        return 0;
    }
    else if (v_type[id] < 6)
    {
        fprintf (stderr, "Erro na linha %d: A fun��o %s t� onde?\n", line_num+1, rem_fname(v_name[id], fname));
        return 0;
    }

    if (get_npar(p_test) != get_npar(v_fpar[id]))
    {
        fprintf(stderr, "Erro na linha %d: lista de par�metros da fun��o %s difere da original.\n", line_num+1, v_name[id]);
        return 0;
    }
...
void par_check(int et)
{
    ...
    if ((t_fun == 1) && (t_cal == 2))
    {
        fprintf(stdout, "Aten��o na linha %d: convertendo float para int no par�metro %d da fun��o %s.\n", line_num+1, index, v_name[fun_id2]);
    ...
    if ((t_fun == 2) && (t_cal == 1))
    {
        fprintf(stdout, "Aten��o na linha %d: convertendo int para float no par�metro %d da fun��o %s.\n", line_num+1, index, v_name[fun_id2]);
